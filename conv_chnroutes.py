...
# thanks to https://github.com/fivesheep/chnroutes
...
def generate_squid(metric):
    rule_name = 'proxyroute'
    results = fetch_ip_data()
    conf_file = open('proxyroute.squid', 'w')
    conf_end = textwrap.dedent("""
    #always_direct allow %s
    """ % (rule_name, ))
    for ip, _, mask in results:
        conf_file.write('acl %s dst %s/%s\n' % (rule_name, ip, mask))

    conf_file.write(conf_end)
    conf_file.close()
...
if __name__=='__main__':
    ...
    elif args.platform.lower() == 'squid':
        generate_squid(args.metric)
    else:
